...
def standard_preprocessing(data, cfg):
    """
...
def load_samples_graph(path, seed_name, seed_id, data_path):
    index_col = 'datetime'
    data = pickle.load(open(os.path.join(path, 'datasets', seed_name, 'east.p'), "rb"))
    data = data[seed_id]
...
def load_joint_samples(cfg, index_col="datetime", preprocessing=None):
    ...
    if cfg.restrict_to >= 0 and cfg.restrict_from >= 0:
        data = data[cfg.restrict_from : cfg.restrict_to + 1]
